...
class conv_transpose_3d_square_input_asym_kernel(Problem):
    """ConvTranspose3D with square input and asymmetric kernel."""
    
    is_exact = False
    
    def __init__(self):
        super().__init__(
            name="conv-transpose-3d-square-input-asym-kernel"
        )
    ...
    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor,
                          ...
                          dilation_d: int, dilation_h: int, dilation_w: int, groups: int) -> torch.Tensor:
    ...
    def generate_test_cases(self, dtype: torch.dtype) -> List[Dict[str, Any]]:
        ...
        return [{
            "name": f"B{batch_size} C{in_channels}->{out_channels} D{depth} W{width} H{height}",
            ...
            "create_inputs": lambda seed=seed, dtype=dtype: (
                *(lambda g: (
                    torch.rand(batch_size, in_channels, depth, width, height, device="cuda", dtype=dtype, generator=g),
                    torch.rand(in_channels, out_channels // groups, kernel_d, kernel_w, kernel_h, device="cuda", dtype=dtype, generator=g),
    ...
    def generate_sample(self, dtype: torch.dtype = torch.float32) -> Dict[str, Any]:
        ...
        return {
            ...
            "create_inputs": lambda: (
                torch.rand(batch_size, in_channels, depth, width, height, device="cuda", dtype=dtype),
                torch.rand(in_channels, out_channels // groups, kernel_d, kernel_w, kernel_h, device="cuda", dtype=dtype),
            )
        }
