...
class matmul_groupnorm_leakyrelu_sum(Problem):
    """Linear projection, GroupNorm, LeakyReLU, sum."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="matmul-groupnorm-leakyrelu-sum"
        )
    ...
    def generate_sample(self, dtype: torch.dtype = torch.float32) -> Dict[str, Any]:
        ...
        return {
            ...
            "create_inputs": lambda: (
                ...
                torch.ones(hidden_size, device="cuda", dtype=dtype),
                torch.zeros(hidden_size, device="cuda", dtype=dtype),
            )
        }
