...
class conv_depthwise_separable_2d(Problem):
    ...
    def __init__(self):
        super().__init__(
            name="conv-depthwise-separable-2d"
        )

    def reference_solution(self, input_tensor: torch.Tensor, depthwise_kernel: torch.Tensor,
                          pointwise_kernel: torch.Tensor, stride_h: int, stride_w: int,
                          padding_h: int, padding_w: int, dilation_h: int, dilation_w: int,
        ...
        return [{
            ...
            "create_inputs": lambda seed=seed, dtype=dtype: (
                *(lambda g: (
                    ...
                    torch.rand(in_channels, in_channels // groups, kernel_h, kernel_w, device="cuda", dtype=dtype, generator=g),
                    torch.rand(out_channels, in_channels, 1, 1, device="cuda", dtype=dtype, generator=g),
    ...
    def generate_sample(self, dtype: torch.dtype = torch.float32) -> Dict[str, Any]:
        ...
        return {
            ...
            "create_inputs": lambda: (
                ...
                torch.ones(in_channels, in_channels // groups, kernel_h, kernel_w, device="cuda", dtype=dtype),
                torch.ones(out_channels, in_channels, 1, 1, device="cuda", dtype=dtype),
            )
        }
    ...
    def get_function_signature(self) -> Dict[str, Any]:
        return {
            "argtypes": [
                ...
                ctypes.POINTER(ctypes.c_float),  # depthwise kernel
                ctypes.POINTER(ctypes.c_float),  # pointwise kernel
    ...
    def get_flops(self, test_case: Dict[str, Any]) -> int:
        ...
        depthwise_flops = 2 * batch_size * in_channels * height * width * (in_channels // groups) * kernel_h * kernel_w
        pointwise_flops = 2 * batch_size * out_channels * height * width * in_channels
