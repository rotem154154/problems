...
class conv2d_groupnorm_tanh_hardswish_residualadd_logsumexp(Problem):
    ...
    def __init__(self):
        super().__init__(
            name="conv2d-groupnorm-tanh-hardswish-residualadd-logsumexp"
        )
    ...
    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor, conv_bias: torch.Tensor,
        ...
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_tensor.dtype):
            ...
            return torch.logsumexp(residual, dim=1, keepdim=True)
