...
class conv_transpose_3d_square_input_sq_kernel(Problem):
    """ConvTranspose3D with square input and square kernel."""
    
    is_exact = False
    
    def __init__(self):
        super().__init__(
            name="conv-transpose-3d-square-input-sq-kernel"
        )
    ...
    def generate_test_cases(self, dtype: torch.dtype) -> List[Dict[str, Any]]:
        ...
        seed = Problem.get_seed(
            f"{self.name}_b={batch_size}_c={in_channels}_d={depth}_h={height}_w={width}_k={kernel_d}"
        )
        
        return [{
            "name": f"B{batch_size} C{in_channels}->{out_channels} D{depth} H{height} W{width} K{kernel_d}",
            ...
            "create_inputs": lambda seed=seed, dtype=dtype: (
                *(lambda g: (
                    ...
                    torch.rand(in_channels, out_channels // groups, kernel_d, kernel_h, kernel_w, device="cuda", dtype=dtype, generator=g),
    ...
    def generate_sample(self, dtype: torch.dtype = torch.float32) -> Dict[str, Any]:
        ...
        return {
            ...
            "create_inputs": lambda: (
                ...
                torch.rand(in_channels, out_channels // groups, kernel_d, kernel_h, kernel_w, device="cuda", dtype=dtype),
            )
        }
