...
class conv2d_avgpool_sigmoid_sum(Problem):
    """Conv2D, avg pool, sigmoid, sum."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv2d-avgpool-sigmoid-sum"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor, bias: torch.Tensor,
                          pool_kernel: int) -> torch.Tensor:
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_tensor.dtype):
            ...
            return torch.sum(out, dim=(1, 2, 3))
