...
class hard_tanh(Problem):
    """HardTanh activation function problem."""
    ...
    def reference_solution(self, input_matrix: torch.Tensor) -> torch.Tensor:
        """
        PyTorch implementation of HardTanh.

        Args:
            input_matrix: Input matrix of shape (M, N)

        Returns:
            Result of HardTanh activation
        """
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_matrix.dtype):
            return torch.nn.functional.hardtanh(input_matrix, min_val=-1.0, max_val=1.0)

    def generate_test_cases(self, dtype: torch.dtype) -> List[Dict[str, Any]]:
        """
        Generate test cases for HardTanh.
        ...
        for name, m, n in test_configs:
            ...
            test_cases.append({
                ...
                "create_inputs": lambda m=m, n=n, seed=seed, dtype=dtype: (
                    *(lambda g: (
                        torch.randn((m, n), device="cuda", dtype=dtype, generator=g) * 3.0,
    ...
    def generate_sample(self, dtype: torch.dtype = torch.float32) -> Dict[str, Any]:
        ...
        return {
            ...
            "create_inputs": lambda m=m, n=n: (
                torch.tensor([[-2.0, -1.0, -0.5, 0.0],
                              [0.5, 1.0, 2.0, -1.5],
                              [1.5, 0.3, -0.3, 3.0],
                              [-3.0, 0.99, -0.99, 0.01]], device="cuda", dtype=dtype),
            )
        }
    ...
    def verify_result(self, expected_output: torch.Tensor,
        ...
        """
        Verify if the HardTanh result is correct.
        ...
        """
        # HardTanh is exact (piecewise linear with simple clamp), so use tight tolerances
    ...
    def get_function_signature(self) -> Dict[str, Any]:
        """
        Get the function signature for the HardTanh solution.
    ...
    def get_flops(self, test_case: Dict[str, Any]) -> int:
        ...
        For hardtanh (clamp), per element:
        - 2 comparisons (x < -1, x > 1)
        ...
        # 2 FLOPs per element: two comparisons/clamp operations
        return M * N * 2
