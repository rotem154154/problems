...
class conv2d_subtract_hardswish_maxpool_mish(Problem):
    """Conv2D followed by subtract, HardSwish, MaxPool, and Mish."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv2d-subtract-hardswish-maxpool-mish"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor,
                          subtract_value: float, pool_kernel: int) -> torch.Tensor:
    ...
    def get_function_signature(self) -> Dict[str, Any]:
        return {
            "argtypes": [
                ...
                ctypes.c_double,                 # subtract_value
