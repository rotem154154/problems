...
class conv_transpose3d_multiply_max_globalavgpool_clamp(Problem):
    """ConvTranspose3D, scale, max pool, global avg pool, clamp."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv-transpose3d-multiply-max-globalavgpool-clamp"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor, conv_bias: torch.Tensor,
                          scale: float, maxpool_kernel: int,
