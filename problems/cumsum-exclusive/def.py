...
class cumsum_exclusive(Problem):
    """Exclusive cumulative sum along a specified dimension."""
    ...
    def reference_solution(self, input_tensor: torch.Tensor, dim: int) -> torch.Tensor:
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_tensor.dtype):
            cumsum = torch.cumsum(
                input_tensor.narrow(dim=dim, start=0, length=input_tensor.size(dim) - 1),
                dim=dim
            )
            leading_zeros = torch.zeros_like(input_tensor.select(dim, 0).unsqueeze(dim))
            return torch.cat((leading_zeros, cumsum), dim=dim)
