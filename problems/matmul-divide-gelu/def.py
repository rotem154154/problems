...
class matmul_divide_gelu(Problem):
    """Linear projection, divide, GELU."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="matmul-divide-gelu"
        )
    ...
    def generate_test_cases(self, dtype: torch.dtype) -> List[Dict[str, Any]]:
        ...
        seed = Problem.get_seed(
            f"{self.name}_b={batch_size}_in={input_size}_out={output_size}"
        )

        return [{
            "name": f"B{batch_size} F{input_size}->{output_size}",
            ...
            "create_inputs": lambda seed=seed, dtype=dtype: (
                *(lambda g: (
                    ...
                    torch.rand(output_size, input_size, device="cuda", dtype=dtype, generator=g),
    ...
    def generate_sample(self, dtype: torch.dtype = torch.float32) -> Dict[str, Any]:
        ...
        return {
            ...
            "create_inputs": lambda: (
                ...
                torch.rand(output_size, input_size, device="cuda", dtype=dtype),
