...
class conv3d_min_softmax(Problem):
    """Conv3D followed by min reduction and softmax."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv3d-min-softmax"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor, min_dim: int) -> torch.Tensor:
