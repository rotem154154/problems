...
class conv_transpose3d_relu_groupnorm(Problem):
    """ConvTranspose3D followed by ReLU and GroupNorm."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv-transpose3d-relu-groupnorm"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor,
                          gn_weight: torch.Tensor, gn_bias: torch.Tensor, groups: int) -> torch.Tensor:
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_tensor.dtype):
            ...
            out = torch.relu(out)
            return torch.nn.functional.group_norm(out, num_groups=groups, weight=gn_weight, bias=gn_bias)
