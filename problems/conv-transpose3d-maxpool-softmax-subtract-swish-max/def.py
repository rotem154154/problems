...
class conv_transpose3d_maxpool_softmax_subtract_swish_max(Problem):
    """ConvTranspose3D, max pool, softmax, subtract, swish, max."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv-transpose3d-maxpool-softmax-subtract-swish-max"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor, conv_bias: torch.Tensor,
                          subtract: torch.Tensor, stride_d: int, stride_h: int, stride_w: int,
                          padding_d: int, padding_h: int, padding_w: int,
                          output_padding_d: int, output_padding_h: int, output_padding_w: int,
                          pool_kernel: int, pool_stride: int, pool_padding: int) -> torch.Tensor:
    ...
    def get_function_signature(self) -> Dict[str, Any]:
        return {
            "argtypes": [
                ...
                ctypes.POINTER(ctypes.c_float),  # subtract
                ...
                ctypes.c_size_t,                 # pool_stride
                ctypes.c_size_t                  # pool_padding
            ],
            "restype": None
        }
    ...
    def get_extra_params(self, test_case: Dict[str, Any]) -> List[Any]:
        return [
            ...
            test_case["pool_stride"],
            test_case["pool_padding"],
        ]
