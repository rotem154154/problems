...
class conv_transpose3d_avgpool_clamp_softmax_multiply(Problem):
    """AvgPool, ConvTranspose3D, clamp, softmax, multiply."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv-transpose3d-avgpool-clamp-softmax-multiply"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor, scale: torch.Tensor,
                          ...
                          pool_kernel: int, clamp_min: float, clamp_max: float) -> torch.Tensor:
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_tensor.dtype):
            out = torch.nn.functional.avg_pool3d(input_tensor, kernel_size=pool_kernel)
        ...
        return [{
            ...
            "create_inputs": lambda seed=seed, dtype=dtype: (
                *(lambda g: (
                    ...
                    torch.ones(1, out_channels, 1, 1, 1, device="cuda", dtype=dtype, generator=g),
    ...
    def generate_sample(self, dtype: torch.dtype = torch.float32) -> Dict[str, Any]:
        ...
        return {
            ...
            "create_inputs": lambda: (
                ...
                torch.ones(1, out_channels, 1, 1, 1, device="cuda", dtype=dtype),
            )
        }
    ...
    def get_function_signature(self) -> Dict[str, Any]:
        return {
            "argtypes": [
                ...
                ctypes.c_double,                 # clamp_min
                ctypes.c_double                  # clamp_max
            ],
            "restype": None
        }
