...
class matmul_swish_sum_groupnorm(Problem):
    """Linear projection, Swish, bias add, and GroupNorm."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="matmul-swish-sum-groupnorm"
        )

    def reference_solution(self, input_tensor: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor,
                          gn_weight: torch.Tensor, gn_bias: torch.Tensor, num_groups: int) -> torch.Tensor:
