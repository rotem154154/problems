...
class conv3d_multiply_instancenorm_clamp_multiply_max(Problem):
    """Conv3D, multiply, InstanceNorm, clamp, multiply, max."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv3d-multiply-instancenorm-clamp-multiply-max"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor, conv_bias: torch.Tensor,
                          multiplier: torch.Tensor, clamp_min: float, clamp_max: float) -> torch.Tensor:
    ...
    def generate_sample(self, dtype: torch.dtype = torch.float32) -> Dict[str, Any]:
        ...
        return {
            ...
            "create_inputs": lambda: (
                ...
                torch.ones(out_channels, 1, 1, 1, device="cuda", dtype=dtype),
            )
        }
    ...
    def get_function_signature(self) -> Dict[str, Any]:
        return {
            "argtypes": [
                ...
                ctypes.POINTER(ctypes.c_float),  # multiplier
