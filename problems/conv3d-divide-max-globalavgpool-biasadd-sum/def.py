...
class conv3d_divide_max_globalavgpool_biasadd_sum(Problem):
    """Conv3D then divide, max pool, global avg pool, bias add, sum."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv3d-divide-max-globalavgpool-biasadd-sum"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor, bias: torch.Tensor,
                          divisor: float, pool_size: int, sum_dim: int) -> torch.Tensor:
    ...
    def get_function_signature(self) -> Dict[str, Any]:
        return {
            "argtypes": [
                ...
                ctypes.c_size_t,                 # pool_size
                ctypes.c_size_t                  # sum_dim
            ],
            "restype": None
        }
