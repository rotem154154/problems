...
class gemm_batchnorm_gelu_relu(Problem):
    """GEMM followed by BatchNorm, GELU, and ReLU."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="gemm-batchnorm-gelu-relu"
        )

    def reference_solution(self, input_tensor: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor,
                          bn_weight: torch.Tensor, bn_bias: torch.Tensor) -> torch.Tensor:
