...
class conv_depthwise_2d_square_input_asym_kernel(Problem):
    ...
    def __init__(self):
        super().__init__(
            name="conv-depthwise-2d-square-input-asym-kernel"
        )
    ...
    def generate_test_cases(self, dtype: torch.dtype) -> List[Dict[str, Any]]:
        ...
        seed = Problem.get_seed(
            f"{self.name}_b={batch_size}_c={in_channels}_h={height}_w={width}_k={kernel_h}x{kernel_w}"
        )

        return [{
            "name": f"B{batch_size} C{in_channels}->{out_channels} H{height} W{width} K{kernel_h}x{kernel_w}",
            ...
            "create_inputs": lambda seed=seed, dtype=dtype: (
                *(lambda g: (
                    ...
                    torch.rand(out_channels, in_channels // groups, kernel_h, kernel_w, device="cuda", dtype=dtype, generator=g),
    ...
    def generate_sample(self, dtype: torch.dtype = torch.float32) -> Dict[str, Any]:
        ...
        return {
            ...
            "create_inputs": lambda: (
                ...
                torch.ones(out_channels, in_channels // groups, kernel_h, kernel_w, device="cuda", dtype=dtype),
            )
        }
    ...
    def get_flops(self, test_case: Dict[str, Any]) -> int:
        ...
        return 2 * batch_size * out_channels * height * width * (in_channels // groups) * kernel_h * kernel_w
