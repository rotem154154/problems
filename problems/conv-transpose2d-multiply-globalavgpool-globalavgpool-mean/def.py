...
class conv_transpose2d_multiply_globalavgpool_globalavgpool_mean(Problem):
    """ConvTranspose2D followed by multiply and two global avg pools."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv-transpose2d-multiply-globalavgpool-globalavgpool-mean"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor,
                          multiplier: float, stride_h: int, stride_w: int,
                          padding_h: int, padding_w: int, output_padding_h: int, output_padding_w: int) -> torch.Tensor:
