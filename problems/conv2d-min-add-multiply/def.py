...
class conv2d_min_add_multiply(Problem):
    """Conv2D then min with scalar, add bias, and scale."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv2d-min-add-multiply"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor, bias: torch.Tensor,
                          constant_value: float, scaling_factor: float) -> torch.Tensor:
