...
class conv2d_groupnorm_scale_maxpool_clamp(Problem):
    """Conv2D, GroupNorm, scaling, max pool, clamp."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv2d-groupnorm-scale-maxpool-clamp"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor, conv_bias: torch.Tensor,
                          gn_weight: torch.Tensor, gn_bias: torch.Tensor, scale: torch.Tensor,
                          num_groups: int, pool_kernel: int, clamp_min: float, clamp_max: float) -> torch.Tensor:
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_tensor.dtype):
            ...
            return torch.clamp(out, min=clamp_min, max=clamp_max)
    ...
    def generate_sample(self, dtype: torch.dtype = torch.float32) -> Dict[str, Any]:
        ...
        return {
            ...
            "create_inputs": lambda: (
                ...
                torch.zeros(out_channels, device="cuda", dtype=dtype),
                torch.ones(out_channels, 1, 1, device="cuda", dtype=dtype),
            )
        }
