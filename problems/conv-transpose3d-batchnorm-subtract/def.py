...
class conv_transpose3d_batchnorm_subtract(Problem):
    """ConvTranspose3D followed by BatchNorm and spatial mean subtraction."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv-transpose3d-batchnorm-subtract"
        )
    ...
    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor,
        ...
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_tensor.dtype):
            ...
            return out - torch.mean(out, dim=(2, 3, 4), keepdim=True)
