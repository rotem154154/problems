...
class gemm_groupnorm_min_biasadd(Problem):
    """GEMM, GroupNorm, min reduction, bias add."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="gemm-groupnorm-min-biasadd"
        )

    def reference_solution(self, input_tensor: torch.Tensor, weight: torch.Tensor, linear_bias: torch.Tensor,
                          gn_weight: torch.Tensor, gn_bias: torch.Tensor, add_bias: torch.Tensor,
                          num_groups: int) -> torch.Tensor:
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_tensor.dtype):
            out = torch.nn.functional.linear(input_tensor, weight, linear_bias)
        ...
        return [{
            ...
            "create_inputs": lambda seed=seed, dtype=dtype: (
                *(lambda g: (
                    ...
                    torch.rand(out_features, device="cuda", dtype=dtype, generator=g),
                    torch.rand(out_features, device="cuda", dtype=dtype, generator=g),
                    torch.rand(1, out_features, 1, 1, device="cuda", dtype=dtype, generator=g),
    ...
    def generate_sample(self, dtype: torch.dtype = torch.float32) -> Dict[str, Any]:
        ...
        return {
            ...
            "create_inputs": lambda: (
                ...
                torch.ones(out_features, device="cuda", dtype=dtype),
                torch.zeros(out_features, device="cuda", dtype=dtype),
                torch.zeros(1, out_features, 1, 1, device="cuda", dtype=dtype),
            )
        }
    ...
    def get_function_signature(self) -> Dict[str, Any]:
        return {
            "argtypes": [
                ...
                ctypes.POINTER(ctypes.c_float),  # linear_bias
