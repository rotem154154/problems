...
class gemm_scale_batchnorm_alt(Problem):
    """GEMM followed by scaling and BatchNorm (alt config)."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="gemm-scale-batchnorm-alt"
        )

    def reference_solution(self, input_tensor: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor,
                          scale: torch.Tensor, bn_weight: torch.Tensor, bn_bias: torch.Tensor) -> torch.Tensor:
