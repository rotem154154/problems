...
class matmul_batchnorm_biasadd_divide_swish(Problem):
    """Linear projection, BatchNorm, bias add, divide, Swish."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="matmul-batchnorm-biasadd-divide-swish"
        )

    def reference_solution(self, input_tensor: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor,
                          bn_weight: torch.Tensor, bn_bias: torch.Tensor, add_bias: torch.Tensor,
        ...
        return [{
            ...
            "create_inputs": lambda seed=seed, dtype=dtype: (
                *(lambda g: (
                    ...
                    torch.rand(out_features, device="cuda", dtype=dtype, generator=g),
                    torch.rand(out_features, device="cuda", dtype=dtype, generator=g),
                    torch.rand(1, device="cuda", dtype=dtype, generator=g),
