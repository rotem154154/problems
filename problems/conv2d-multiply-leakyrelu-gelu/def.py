...
class conv2d_multiply_leakyrelu_gelu(Problem):
    """Conv2D then multiply, LeakyReLU, GELU."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv2d-multiply-leakyrelu-gelu"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor, bias: torch.Tensor,
                          multiplier: torch.Tensor) -> torch.Tensor:
