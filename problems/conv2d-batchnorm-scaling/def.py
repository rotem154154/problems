...
class conv2d_batchnorm_scaling(Problem):
    """Conv2D, BatchNorm, scaling."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv2d-batchnorm-scaling"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor, bias: torch.Tensor,
                          bn_weight: torch.Tensor, bn_bias: torch.Tensor, scaling_factor: float) -> torch.Tensor:
