...
class conv_transpose3d_sum_layernorm_avgpool_gelu(Problem):
    """ConvTranspose3D followed by sum, layer norm, avg pool, and GELU."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv-transpose3d-sum-layernorm-avgpool-gelu"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor, sum_weight: float,
                          stride_d: int, stride_h: int, stride_w: int, padding_d: int, padding_h: int, padding_w: int,
