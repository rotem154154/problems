...
class matmul_transposed_b(Problem):
    """Matrix multiplication with transposed B problem."""
    
    is_exact = False
    
    def __init__(self):
        super().__init__(
            name="matmul-transposed-b"
        )
    
    def reference_solution(self, input_a: torch.Tensor, input_b: torch.Tensor) -> torch.Tensor:
        """
        PyTorch implementation of matrix multiplication with B transposed.
        ...
        Returns:
            Result of A * B.T with shape (M, N)
        """
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_a.dtype):
            return torch.matmul(input_a, input_b.T)
    
    def generate_test_cases(self, dtype: torch.dtype) -> List[Dict[str, Any]]:
        """
        Generate test cases for matrix multiplication with transposed B.
    ...
    def generate_sample(self, dtype: torch.dtype = torch.float32) -> Dict[str, Any]:
        """
        Generate sample test case for transposed-B matrix multiplication.
