...
class conv_transpose3d_add_hardswish(Problem):
    """ConvTranspose3D add input then HardSwish-based gating."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv-transpose3d-add-hardswish"
        )

    def reference_solution(self, input_tensor: torch.Tensor, add_input: torch.Tensor, kernel: torch.Tensor,
        ...
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_tensor.dtype):
            ...
            out = out + add_input
            return out * torch.nn.functional.hardswish(out)
        ...
        return [{
            ...
            "create_inputs": lambda seed=seed, dtype=dtype: (
                *(lambda g: (
                    ...
                    torch.rand(batch_size, out_channels, depth * stride_d, height * stride_h, width * stride_w, device="cuda", dtype=dtype, generator=g),
    ...
    def generate_sample(self, dtype: torch.dtype = torch.float32) -> Dict[str, Any]:
        ...
        return {
            ...
            "create_inputs": lambda: (
                ...
                torch.rand(batch_size, out_channels, depth * stride_d, height * stride_h, width * stride_w, device="cuda", dtype=dtype),
    ...
    def get_function_signature(self) -> Dict[str, Any]:
        return {
            "argtypes": [
                ...
                ctypes.POINTER(ctypes.c_float),  # add_input
