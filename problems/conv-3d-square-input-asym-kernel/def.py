...
class conv_3d_square_input_asym_kernel(Problem):
    ...
    def __init__(self):
        super().__init__(
            name="conv-3d-square-input-asym-kernel"
        )
    ...
    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor,
                          ...
                          dilation_d: int, dilation_h: int, dilation_w: int) -> torch.Tensor:
    ...
    def generate_test_cases(self, dtype: torch.dtype) -> List[Dict[str, Any]]:
        ...
        seed = Problem.get_seed(
            f"{self.name}_b={batch_size}_c={in_channels}_h={height}_w={width}_d={depth}"
        )
        
        return [{
            "name": f"B{batch_size} C{in_channels}->{out_channels} W{width} H{height} D{depth}",
            ...
            "create_inputs": lambda seed=seed, dtype=dtype: (
                *(lambda g: (
                    torch.rand(batch_size, in_channels, width, height, depth, device="cuda", dtype=dtype, generator=g),
    ...
    def generate_sample(self, dtype: torch.dtype = torch.float32) -> Dict[str, Any]:
        ...
        return {
            ...
            "create_inputs": lambda: (
                torch.rand(batch_size, in_channels, width, height, depth, device="cuda", dtype=dtype),
    ...
    def get_flops(self, test_case: Dict[str, Any]) -> int:
        ...
        return 2 * batch_size * out_channels * height * width * depth * in_channels * kernel_d * kernel_h * kernel_w
