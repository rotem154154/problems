...
class conv3d_scaling_tanh_multiply_sigmoid(Problem):
    """Conv3D then scale, tanh, multiply, sigmoid."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv3d-scaling-tanh-multiply-sigmoid"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor,
                          scale: torch.Tensor, bias: torch.Tensor) -> torch.Tensor:
