...
class conv2d_tanh_scaling_biasadd_max(Problem):
    """Conv2D, tanh, scaling, bias add, max pool."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv2d-tanh-scaling-biasadd-max"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor, conv_bias: torch.Tensor,
                          add_bias: torch.Tensor, scaling_factor: float, pool_kernel: int) -> torch.Tensor:
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_tensor.dtype):
            ...
            out = out + add_bias
            return torch.nn.functional.max_pool2d(out, kernel_size=pool_kernel)
