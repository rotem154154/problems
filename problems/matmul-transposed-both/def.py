...
class matmul_transposed_both(Problem):
    """Matrix multiplication with both inputs transposed problem."""
    
    is_exact = False
    
    def __init__(self):
        super().__init__(
            name="matmul-transposed-both"
        )
    
    def reference_solution(self, input_a: torch.Tensor, input_b: torch.Tensor) -> torch.Tensor:
        """
        PyTorch implementation of matrix multiplication with both inputs transposed.
        
        Args:
            input_a: Input matrix A of shape (K, M)
            input_b: Input matrix B of shape (N, K)
            
        Returns:
            Result of A.T * B.T with shape (M, N)
        """
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_a.dtype):
            return torch.matmul(input_a.T, input_b.T)
    
    def generate_test_cases(self, dtype: torch.dtype) -> List[Dict[str, Any]]:
        """
        Generate test cases for matrix multiplication with both inputs transposed.
        ...
        return [{
            "name": f"{k}x{m}^T x {n}x{k}^T",
            "dims": (m, n, k),
            "create_inputs": lambda m=m, n=n, k=k, seed=seed, dtype=dtype: (
                *(lambda g: (
                    torch.rand(k, m, device="cuda", dtype=dtype, generator=g),
                    torch.rand(n, k, device="cuda", dtype=dtype, generator=g),
    ...
    def generate_sample(self, dtype: torch.dtype = torch.float32) -> Dict[str, Any]:
        """
        Generate sample test case for transposed-both matrix multiplication.
        ...
        return {
            ...
            "create_inputs": lambda m=m, n=n, k=k: (
                torch.arange(1, k * m + 1, device="cuda", dtype=dtype).view(k, m),
                torch.arange(1, n * k + 1, device="cuda", dtype=dtype).view(n, k),
            )
        }
