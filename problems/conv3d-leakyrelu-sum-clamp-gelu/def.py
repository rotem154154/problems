...
class conv3d_leakyrelu_sum_clamp_gelu(Problem):
    """Conv3D, LeakyReLU, sum, clamp, GELU."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv3d-leakyrelu-sum-clamp-gelu"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor, conv_bias: torch.Tensor,
                          sum_tensor: torch.Tensor) -> torch.Tensor:
    ...
    def get_function_signature(self) -> Dict[str, Any]:
        return {
            "argtypes": [
                ...
                ctypes.POINTER(ctypes.c_float),  # sum_tensor
