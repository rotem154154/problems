...
class conv_transpose2d_batchnorm_tanh_maxpool_groupnorm(Problem):
    """ConvTranspose2D followed by BatchNorm, Tanh, MaxPool, and GroupNorm."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv-transpose2d-batchnorm-tanh-maxpool-groupnorm"
        )
    ...
    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor,
                          ...
                          output_padding_h: int, output_padding_w: int, num_groups: int) -> torch.Tensor:
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_tensor.dtype):
            ...
            return torch.nn.functional.group_norm(out, num_groups=num_groups, weight=gn_weight, bias=gn_bias)
