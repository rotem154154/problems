...
class matmul_maxpool_sum_scale(Problem):
    """Linear projection, max pool, sum, scale."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="matmul-maxpool-sum-scale"
        )

    def reference_solution(self, input_tensor: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor,
                          kernel_size: int, scale_factor: float) -> torch.Tensor:
