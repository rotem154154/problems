...
class matmul_scale_residualadd_clamp_logsumexp_mish(Problem):
    """Linear projection followed by scale, residual add, clamp, logsumexp, and Mish."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="matmul-scale-residualadd-clamp-logsumexp-mish"
        )

    def reference_solution(self, input_tensor: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor,
                          scale_factor: float, clamp_min: float, clamp_max: float) -> torch.Tensor:
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_tensor.dtype):
            ...
            return out * torch.nn.functional.mish(out)
