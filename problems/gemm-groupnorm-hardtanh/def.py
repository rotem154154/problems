...
class gemm_groupnorm_hardtanh(Problem):
    """GEMM followed by GroupNorm and Hardtanh."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="gemm-groupnorm-hardtanh"
        )
    ...
    def reference_solution(self, input_tensor: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor,
        ...
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_tensor.dtype):
            ...
            return torch.nn.functional.hardtanh(out, min_val=hardtanh_min, max_val=hardtanh_max)
