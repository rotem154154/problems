...
class conv_pointwise_2d(Problem):
    ...
    def generate_test_cases(self, dtype: torch.dtype) -> List[Dict[str, Any]]:
        ...
        seed = Problem.get_seed(
            f"{self.name}_b={batch_size}_c={in_channels}_h={height}_w={width}_k=1"
        )

        return [{
            "name": f"B{batch_size} C{in_channels}->{out_channels} H{height} W{width} K1",
    ...
    def generate_sample(self, dtype: torch.dtype = torch.float32) -> Dict[str, Any]:
        ...
        return {
            ...
            "create_inputs": lambda: (
                ...
                torch.ones(out_channels, in_channels, kernel_h, kernel_w, device="cuda", dtype=dtype),
            )
        }
