...
class conv_transpose3d_batchnorm_avgpool_avgpool(Problem):
    """ConvTranspose3D, BatchNorm, AvgPool3D twice."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv-transpose3d-batchnorm-avgpool-avgpool"
        )
    ...
    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor, conv_bias: torch.Tensor,
        ...
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_tensor.dtype):
            ...
            return torch.nn.functional.avg_pool3d(out, kernel_size=2)
