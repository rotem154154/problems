...
class matmul_sum_max_avgpool_logsumexp_logsumexp(Problem):
    """Linear projection followed by reductions and logsumexp."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="matmul-sum-max-avgpool-logsumexp-logsumexp"
        )
    ...
    def reference_solution(self, input_tensor: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor) -> torch.Tensor:
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_tensor.dtype):
            ...
            return torch.logsumexp(out, dim=1, keepdim=True)
