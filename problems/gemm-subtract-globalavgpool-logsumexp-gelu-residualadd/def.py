...
class gemm_subtract_globalavgpool_logsumexp_gelu_residualadd(Problem):
    """GEMM, subtract, global avg pool, logsumexp, GELU, residual add."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="gemm-subtract-globalavgpool-logsumexp-gelu-residualadd"
        )

    def reference_solution(self, input_tensor: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor,
                          subtract: torch.Tensor) -> torch.Tensor:
