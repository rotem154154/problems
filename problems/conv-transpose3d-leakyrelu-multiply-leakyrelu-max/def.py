...
class conv_transpose3d_leakyrelu_multiply_leakyrelu_max(Problem):
    """ConvTranspose3D, LeakyReLU, multiply, LeakyReLU, MaxPool3D."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="conv-transpose3d-leakyrelu-multiply-leakyrelu-max"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor, conv_bias: torch.Tensor,
                          multiplier: torch.Tensor, stride_d: int, stride_h: int, stride_w: int,
