...
class conv_transpose2d_add_min_gelu_multiply(Problem):
    ...
    def __init__(self):
        super().__init__(
            name="conv-transpose2d-add-min-gelu-multiply"
        )

    def reference_solution(self, input_tensor: torch.Tensor, kernel: torch.Tensor, conv_bias: torch.Tensor,
                          add_value: float, multiply_value: float, stride_h: int, stride_w: int) -> torch.Tensor:
    ...
    def get_function_signature(self) -> Dict[str, Any]:
        return {
            "argtypes": [
                ...
                ctypes.c_double,                 # add_value
                ctypes.c_double,                 # multiply_value
