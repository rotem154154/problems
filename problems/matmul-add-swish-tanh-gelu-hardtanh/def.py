...
class matmul_add_swish_tanh_gelu_hardtanh(Problem):
    """Linear projection, add, Swish, Tanh, GELU, Hardtanh."""

    is_exact = False

    def __init__(self):
        super().__init__(
            name="matmul-add-swish-tanh-gelu-hardtanh"
        )

    def reference_solution(self, input_tensor: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor,
                          add_value: torch.Tensor) -> torch.Tensor:
        with torch.no_grad(), torch.autocast("cuda", enabled=False, dtype=input_tensor.dtype):
            ...
            return torch.nn.functional.hardtanh(out, min_val=-1.0, max_val=1.0)
    ...
    def get_function_signature(self) -> Dict[str, Any]:
        return {
            "argtypes": [
                ...
                ctypes.POINTER(ctypes.c_float),  # add_value
